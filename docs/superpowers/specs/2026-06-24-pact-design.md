...
- **Outbound channel:** coaching nudges are persisted to an **outbox** (`GET /api/outbox`) and **delivered through the Hermes agent's own channel** — the user already talks to their agent there, so no separate transport is needed. Pact owns content and timing; the agent owns delivery. The agent fetches undelivered nudges, relays each one, then calls `POST /api/coach/{id}/delivered` to mark it done. The web thread also shows the coaching log so nudges are visible on both surfaces.
...
- **Day 3 — Hermes-native + real rails:** the reasoning-task broker + `/pact serve` worker so a real Hermes agent is the brain (skill reasons inline; website tasks claimed by a connected capable agent; `test_llm` stays the hybrid fallback); modality capability negotiation; agent-channel outbox delivery; scheduler/reconciler ticker.

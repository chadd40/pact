...
def test_save_and_get_profile_round_trips(repo: Repository) -> None:
    ...
    loaded = repo.get_profile("demo@pact.local")
...
def test_reset_all_empties_every_table(repo: Repository) -> None:
    ...
    repo.save_profile(Profile(owner="demo@pact.local", pact_ids=["pact_abc123"]))
    ...
    assert repo.get_profile("demo@pact.local") is not None
    ...
    assert repo.get_profile("demo@pact.local") is None
    assert repo.list_coaching_messages("pact_abc123") == []
